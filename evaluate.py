...
def all_metrics(yhat, y, k=8, yhat_raw=None):
    ...
    if yhat_raw is not None:
        ...
        for k_i in k:
            ...
            ht_at_k_val, ht_at_k = hit_at_k(yhat_raw, yhat, y, k_i)
            metrics['hit_at_%d' % k_i] = ht_at_k
            ht_at_1_val, ht_at_1 = hit_at_k(yhat_raw, yhat, y, 1)
            metrics['hit_at_1'] = ht_at_1
            

        roc_auc = auc_metrics(yhat_raw, y, ymic)
        metrics.update(roc_auc)

    return metrics, ht_at_k_val, ht_at_1_val
...
def hit_at_k(yhat_raw, yhat, y, k):
    """
    Assume only one true label
    if top k predictions have intersection with target, return 1 else 0
    """
    sortd = np.argsort(yhat_raw)[:,::-1]
    topk = sortd[:,:k]
    ...
    for i, tk in enumerate(topk):
        if len(tk) > 0:
            num_true_in_top_k = y[i,tk].sum()
            vals.append(float(num_true_in_top_k))
...
def print_metrics(metrics):
    ...
    if have_k:
        ...
        print("hit@%d, hit@1" % k)
        print("%.4f, %.4f" % (metrics["hit_at_%d" % k], metrics["hit_at_1"]))
    print
